...
from gateways.behpardakht_gateway.exceptions import messages
...
class BehpardakhtGateway(BasePaymentGateway):
    ...
    def _get_client(self):
        ...
        return Client(self._default_urls()['payment_wsdl'], transport=transport)
    ...
    def _settle_payment(self, data: dict):
    ...
    def pay(self, info: BehpardakhtInfoDataModel, data: PaySchema) -> PayOutSchema:
        ...
        try:
            ...
            status, token = parts
            if len(parts) != 2:
                raise GatewayConnectionError("پاسخ نامعتبر از بانک دریافت شد.")
            if status == "0":
                return PayOutSchema(
    ...
    def after_pay(self, data: AfterPaySchema) -> GatewayTransactionDataModel:
        ...
        card_number = data.CardHolderPan
        ref_id = data.RefId
